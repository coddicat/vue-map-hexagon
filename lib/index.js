...
var PinchScrollZoomAxis = /** @class */ (function () {
    function PinchScrollZoomAxis(size, origin, translate, contentSize) {
        this._point = 0;
        this._start1 = 0;
        this._start2 = 0;
    ...
    Object.defineProperty(PinchScrollZoomAxis.prototype, "point", {
        get: function () { return this._point; },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(PinchScrollZoomAxis.prototype, "origin", {
        get: function () { return this._origin; },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(PinchScrollZoomAxis.prototype, "start1", {
        get: function () { return this._start1; },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(PinchScrollZoomAxis.prototype, "start2", {
        get: function () { return this._start2; },
        enumerable: false,
        configurable: true
    });
    PinchScrollZoomAxis.prototype.touch = function (point) {
        this._start1 = point - this._point;
    };
    PinchScrollZoomAxis.prototype.pinch = function (point1, point2, scale) {
        var prevOrigin = this._origin;
        var point = (point1 + point2) / 2;
        this._origin = prevOrigin + (point - prevOrigin - this._point) / scale;
        var delta = prevOrigin - this._origin;
        this._point += delta - delta * scale;
        this._start1 = point1 - this._point;
        this._start2 = point2 - this._point;
    };
    PinchScrollZoomAxis.prototype.dragPinch = function (point1, point2) {
        var start = (this._start1 + this._start2) / 2;
        var point = (point1 + point2) / 2;
        this._point = point - start;
    };
    PinchScrollZoomAxis.prototype.dragTouch = function (point) {
        this._point = point - this._start1;
    };
    PinchScrollZoomAxis.prototype.checkAndResetToWithin = function (scale) {
        var _a;
        var contentSize = (_a = this._contentSize) !== null && _a !== void 0 ? _a : this._size;
        if (contentSize * scale < this._size) {
            this._origin = this._size / 2;
            this._point = (this._size - contentSize) * scale / 2;
            return;
        }
        var overflow = (contentSize - this._size) * scale;
        var border = (this._size - this._size * scale) / 2;
        var originFactor = (this._size / 2 - this._origin - (this._size / 2 - this._origin) * scale);
    ...
    PinchScrollZoomAxis.prototype.setPoint = function (point) {
        this._point = point;
    };
    PinchScrollZoomAxis.prototype.setOrigin = function (origin) {
        this._origin = origin;
    };
    return PinchScrollZoomAxis;
}());

var script$2 = /*#__PURE__*/ Vue__default['default'].extend({
    ...
    watch: {
        ...
        originX: function (val) {
            this.axisX.setOrigin(val);
        },
        originY: function (val) {
    ...
    data: function () {
        return {
            ...
            axisX: new PinchScrollZoomAxis(this.width, this.originX, this.translateX, this.contentWidth),
            axisY: new PinchScrollZoomAxis(this.height, this.originY, this.translateY, this.contentHeight),
            throttleDoDrag: lodash.throttle(this.doDragEvent, this.throttleDelay),
            stopScaling: lodash.debounce(this.doStopScalingEvent, 200),
            zoomIn: false,
            zoomOut: false,
    ...
    methods: {
        ...
        startDrag: function (touchEvent) {
            ...
            var clientX1 = this.getBoundingTouchClientX(touchEvent.touches[0]);
            var clientY1 = this.getBoundingTouchClientY(touchEvent.touches[0]);
            if (touchEvent.touches.length > 1) {
                this.touch1 = true;
                this.touch2 = true;
                this.startScale = this.currentScale;
                var clientX2 = this.getBoundingTouchClientX(touchEvent.touches[1]);
                var clientY2 = this.getBoundingTouchClientY(touchEvent.touches[1]);
        ...
        doDrag: function (touchEvent) {
            if (!this.draggable)
                return;
            this.throttleDoDrag(touchEvent);
        },
        doStopScalingEvent: function () {
            this.zoomIn = false;
            this.zoomOut = false;
        },
        doDragEvent: function (touchEvent) {
            ...
            if (!this.touch1 || (!this.touch2 && touchEvent.touches.length === 2))
                this.startDrag(touchEvent);
            if (this.touch1 && this.touch2) {
                this.axisX.dragPinch(this.getBoundingTouchClientX(touchEvent.touches[0]), this.getBoundingTouchClientX(touchEvent.touches[1]));
                this.axisY.dragPinch(this.getBoundingTouchClientY(touchEvent.touches[0]), this.getBoundingTouchClientY(touchEvent.touches[1]));
            }
            else {
        ...
        getBoundingTouchClientX: function (touch) {
            return touch.clientX - this.$el.getBoundingClientRect().left;
        },
        getBoundingTouchClientY: function (touch) {
        ...
        getDistance: function (x1, y1, x2, y2) {
            var sqrDistance = Math.pow((x1 - x2), 2) + Math.pow((y1 - y2), 2);
            var distance = Math.sqrt(sqrDistance);
            return distance;
        },
        doScale: function (touchEvent) {
        ...
        submitScale: function (scale) {
            if ((scale >= this.minScale || this.currentScale < scale) &&
        ...
        doWheelScale: function (event) {
            event.preventDefault();
            var clientX = this.getBoundingTouchClientX(event);
            var clientY = this.getBoundingTouchClientY(event);
            this.axisX.pinch(clientX, clientX, this.currentScale);
            this.axisY.pinch(clientY, clientY, this.currentScale);
            var factor = 1 - event.deltaY * this.wheelVelocity;
            var scale = this.currentScale * factor;
            this.submitScale(scale);
        },
...
var __vue_render__$2 = function() {
  ...
  return _c(
    "div",
    {
      staticClass: "pinch-scroll-zoom",
      class: {
        "pinch-scroll-zoom--zoom-out": _vm.zoomOut,
        "pinch-scroll-zoom--zoom-in": _vm.zoomIn
      },
      style: { width: _vm.width + "px", height: _vm.height + "px" },
      on: { mousedown: _vm.startDrag, mousemove: _vm.doDrag }
    },
    [
      _c(
        "div",
        {
          ref: "content",
          staticClass: "pinch-scroll-zoom__content",
...
var script = /*#__PURE__*/ Vue__default['default'].extend({
    name: "MapHexagon",
    components: {
        PinchScrollZoom: __vue_component__$2,
        Hexagon: __vue_component__$1,
    },
    props: {
        cells: {
            type: Array,
        ...
        xRange: {
            ...
            default: function () {
                ...
                var arr = this.cells.map(function (x) { return x.col; });
        ...
        yRange: {
            ...
            default: function () {
                ...
                var arr = this.cells.map(function (x) { return x.row; });
                var min = Math.min.apply(null, arr);
                var max = Math.max.apply(null, arr);
                return [min, max];
            },
        ...
        center: {
            ...
            default: function () {
                ...
                var x = Math.floor((xRange[0] + xRange[1]) / 2);
                var y = Math.floor((yRange[0] + yRange[1]) / 2);
                return [x, y];
            },
        },
        scale: {
    ...
    computed: {
        ...
        xOffset: function () {
            var cnx = this.getXposition(this.xRange[0]);
            return cnx - this.radius;
        },
        contentWidth: function () {
            var res = (this.xRange[1] - this.xRange[0] + 1) * this.xfactor + this.radius / 2;
            return res;
        },
        yOffset: function () {
            var cny = this.getYposition(0, this.yRange[0]);
            return cny - this.yfactor;
        },
        contentHeight: function () {
            var res = (this.yRange[1] - this.yRange[0] + 1) * this.yfactor * 2 + this.yfactor;
            return res;
        },
        _cells: function () {
            var cols = this.xRange[1] - this.xRange[0] + 1;
            var cells = [];
            var top = this.yRange[0] > this.visibleTop ? this.yRange[0] : this.visibleTop;
            var bottom = this.yRange[1] < this.visibleBottom
                ? this.yRange[1]
                : this.visibleBottom;
            var left = this.xRange[0] > this.visibleLeft ? this.xRange[0] : this.visibleLeft;
            var right = this.xRange[1] < this.visibleRight ? this.xRange[1] : this.visibleRight;
            if (this.cells) {
                return this.cells.filter(function (x) {
                    return x.row >= top && x.row <= bottom && x.col >= left && x.col <= right;
                });
            }
    ...
    methods: {
        onDragging: function (event) {
            ...
            var originX = event.originX;
            var originY = event.originY;
            var scaleFactorX = scale * this.xfactor;
            var floatLeft = (originX * scale - event.x - originX) / scaleFactorX;
            var floatRight = floatLeft + this.width / scaleFactorX;
            this.visibleLeft = Math.floor(floatLeft) + this.xRange[0] - 1;
            this.visibleRight = Math.floor(floatRight) + this.xRange[0];
            var scaleFactorY = scale * this.yfactor * 2;
            var floatTop = (originY * scale - event.y - originY) / scaleFactorY;
            var floatBottom = floatTop + this.height / scaleFactorY;
        ...
        submitCenter: function () {
            ...
            this.$nextTick(function () {
                var xCenter = _this.getXcenter();
                var yCenter = _this.getYcenter();
                _this.$refs.zoomer.setData({
        ...
        submitSize: function () {
            var data = this.$refs.zoomer.getEmitData();
            var event = {
                x: data.translateX,
                y: data.translateY,
        ...
        getXcenter: function () {
            ...
            var xfactor = this.xfactor;
            var middle = (this.xRange[0] + this.xRange[1]) / 2;
            var x = Math.floor(middle);
            var odd = middle % 1 === 0 ? 0 : 1;
            var center = ((xfactor * odd - this.contentWidth) * scale + this.width) / 2;
            var shift = xfactor * scale * (this.center[0] - x);
            return center - shift;
        },
        getYcenter: function () {
            var yfactor = this.yfactor;
            var scale = this.currentScale;
            var center = ((yfactor - this.contentHeight) * scale + this.height) / 2;
            var middle = (this.yRange[0] + this.yRange[1]) / 2;
            var y = Math.floor(middle);
            var odd = this.center[0] % 2 == 0 ? 0 : 0.5;
            var odd_ = middle % 1 === 0 ? 0 : 0.5;
            var shift = 2 * yfactor * (this.center[1] - y - odd_ + odd) * scale;
            return center - shift;
        },
        getXposition: function (col) {
            var nx = col * this.xfactor;
            var cnx = nx + this.contentWidth / 2;
            return cnx;
        },
        getYposition: function (col, row) {
            var fy = col % 2 == 0 ? 0 : 0.5;
            var ny = (row + fy) * this.yfactor * 2;
            var cny = ny + this.contentHeight / 2;
            return cny;
        },
        getPositionStyles: function (col, row) {
            var x = this.getXposition(col);
            var y = this.getYposition(col, row);
            return {
                top: y + "px",
                left: x + "px",
            };
  ...
  return _c(
    "PinchScrollZoom",
    {
      ...
      attrs: {
        ...
        "throttle-delay": 40,
        scale: _vm.scale,
        "min-scale": _vm._minScale,
        "max-scale": _vm._maxScale,
        draggable: _vm.draggable
      },
      on: { scaling: _vm.onScaling, dragging: _vm.onDragging }
    },
    [
      _c(
        "div",
        { staticClass: "map-hexagon", style: _vm.getMapStyles },
        _vm._l(_vm._cells, function(cell) {
          return _c(
            "div",
            {
              key: cell.index,
              staticClass: "map-hexagon__item",
              style: _vm.getPositionStyles(cell.col, cell.row)
            },
            [
              _vm._t(
                "default",
                      ...
                      [
                        cell.text
                          ? _c("span", {
                              ...
                              domProps: { innerHTML: _vm._s(cell.text) }
                            })
                          : _vm._e()
                      ]
                    )
